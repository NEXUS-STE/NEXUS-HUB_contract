...
/// Minimum ledger gap before a client can open a dispute unilaterally
...
impl NexusEscrow {
    ...
    /// Pause all escrow operations. Only callable by admin.
    ...
    /// * `description`    — Short human-readable label (max 256 chars).
    /// * `dispute_delay`     — Extra ledgers beyond MIN_DISPUTE_DELAY before client can dispute.
    /// * `milestones_total`  — Number of milestones for progress tracking; 0 disables tracking.
    /// * `expiry_ledger`     — Absolute ledger at which the escrow can be claimed expired; 0 disables.
    pub fn init_escrow(
        env: Env,
        escrow_id: Bytes,
        client: Address,
    ...
    /// The client must have approved the contract address to spend `amount` tokens.
    pub fn fund(env: Env, escrow_id: Bytes) -> Result<(), EscrowError> {
        ...
        // Pull tokens from client into this contract.
    ...
    /// Client approves work. Sends `amount - platform_fee` to freelancer,
    /// and `platform_fee` to admin. Status: Funded|Active → Released.
    ///
    ...
    pub fn release(
        ...
        // Optional milestone hash verification.
    ...
    /// No platform fee is deducted on refund.
    /// Valid from: Funded, Active, or Disputed.
    pub fn refund(env: Env, escrow_id: Bytes) -> Result<(), EscrowError> {
        ...
        // Only admin can issue refunds directly.
    ...
    pub fn cancel(env: Env, escrow_id: Bytes) -> Result<(), EscrowError> {
        bump_instance(&env);
        let mut record = load_escrow(&env, &escrow_id)?;
        ...
        // Either client or admin can cancel a pending escrow.
        let caller_is_admin = env
            .storage()
            .persistent()
            .get::<types::DataKey, Address>(&types::DataKey::Admin)
            .map(|a| a == record.client)
            .unwrap_or(false);
    ...
    ///
    /// A client may raise a dispute freely.
    /// A freelancer may raise a dispute only after `dispute_deadline_ledger` passes,
    /// preventing premature escalation before the client has had time to review.
    pub fn raise_dispute(env: Env, escrow_id: Bytes, caller: Address) -> Result<(), EscrowError> {
        ...
        // Freelancer must wait for the dispute deadline.
    ...
    /// Admin resolves a dispute by choosing who receives the funds.
    ...
    /// Freelancer marks one milestone as complete. Status must be Funded or Active.
    /// The escrow must have been created with milestones_total > 0.
    ...
    /// No pause check — this is a safety valve that must work even when paused.
    ...
    pub fn approve_release(env: Env, escrow_id: Bytes, approver: Address) -> Result<(), EscrowError> {
        ...
        // Save approval state and signal progress before checking for full release.
        save_escrow(&env, &record);
